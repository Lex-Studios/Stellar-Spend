...
describe("POST /api/webhooks/paycrest (integration)", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(notifyTransactionStatusUpdate)
      .mockReset()
  ...
  it("returns 200 with valid HMAC signature, timestamp, and nonce", async () => {
    const body = JSON.stringify({
      ...
      data: { id: "order-200-test" },
    });
    const sig = await hmacHex(body, "test-webhook-secret");
    const res = await POST(
      makeRequest(body, sig, String(Date.now()), "int-nonce-1"),
    );
  ...
  it("returns 401 with invalid signature", async () => {
    ...
    const res = await POST(
      makeRequest(body, "invalid-sig", String(Date.now()), "int-nonce-2"),
    );
  ...
  it("returns 401 when signature header is missing", async () => {
    ...
    const req = new Request("http://localhost/api/webhooks/paycrest", {
      method: "POST",
      body,
    });
  ...
  it("updates transaction to completed on payment_order.settled", async () => {
    vi.mocked(dal.getByPayoutOrderId).mockResolvedValue(
      FAKE_TRANSACTION as any,
    );
    const body = JSON.stringify({
      event: "payment_order.settled",
  ...
  it("returns 200 without updating when no transaction found for orderId", async () => {
    ...
    const sig = await hmacHex(body, "test-webhook-secret");
    const res = await POST(
      makeRequest(body, sig, String(Date.now()), "int-nonce-7"),
    );
    expect(res.status).toBe(200);
  ...
  it("returns 400 for malformed JSON payload", async () => {
    ...
    const sig = await hmacHex(body, "test-webhook-secret");
    const res = await POST(
      makeRequest(body, sig, String(Date.now()), "int-nonce-9"),
    );
