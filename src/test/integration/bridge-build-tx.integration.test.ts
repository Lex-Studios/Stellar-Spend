...
const FAKE_SOURCE_TOKEN = {
  symbol: "USDC",
  decimals: 7,
  contract: "0xsrc",
  chain: "STELLAR",
};
const FAKE_DEST_TOKEN = {
  symbol: "USDC",
  decimals: 6,
  contract: "0xdst",
  chain: "BASE",
...
describe("POST /api/offramp/bridge/build-tx (integration)", () => {
  ...
  it("returns 400 for invalid Stellar address", async () => {
    vi.mocked(validation.validateAmount).mockReturnValue(true);
    vi.mocked(validation.validateAddress).mockImplementation(
      (_addr, chain) => chain !== "stellar",
    );
    const res = await POST(
  ...
  it("returns 500 with user-friendly message for insufficient balance error", async () => {
    ...
    mockSend.mockRejectedValue(
      new Error("resulting balance is not within the allowed range"),
    );
    const res = await POST(
      makeReq({
        amount: "10",
