...
describe("buildRequestHash", () => {
  ...
  it("handles nested objects consistently", () => {
    const hash1 = buildRequestHash(
      "POST",
      "/api/test",
      '{"nested":{"b":2,"a":1}}',
    );
    const hash2 = buildRequestHash(
      "POST",
      "/api/test",
      '{"nested":{"a":1,"b":2}}',
    );
...
describe("Idempotency contract", () => {
  ...
  it("Idempotency-Key header should be scoped to (key, method, path)", () => {
    ...
    const hashSame1 = buildRequestHash(
      "POST",
      "/api/offramp/paycrest/order",
      body,
    );
    const hashSame2 = buildRequestHash(
      "POST",
      "/api/offramp/paycrest/order",
      body,
    );
    const hashDiffPath = buildRequestHash(
      "POST",
      "/api/offramp/execute-payout",
      body,
    );
    const hashDiffMethod = buildRequestHash(
      "GET",
      "/api/offramp/paycrest/order",
      body,
    );
