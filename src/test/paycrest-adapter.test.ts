...
describe("PaycrestAdapter.getCurrencies", () => {
  ...
  it("throws PaycrestHttpError on non-ok response", async () => {
    ...
    await expect(adapter.getCurrencies()).rejects.toBeInstanceOf(
      PaycrestHttpError,
    );
  });
});
...
describe("PaycrestAdapter.getInstitutions", () => {
  ...
  it("throws PaycrestHttpError on non-ok response", async () => {
    ...
    await expect(adapter.getInstitutions("XYZ")).rejects.toBeInstanceOf(
      PaycrestHttpError,
    );
  });
});
...
describe("PaycrestAdapter.getRate", () => {
  ...
  it("calls the correct path-param endpoint", async () => {
    ...
    await adapter.getRate("USDC", "50", "NGN", {
      network: "base",
      providerId: "p1",
    });
  ...
  it("throws when rate is not finite (NaN)", async () => {
    ...
    await expect(adapter.getRate("USDC", "100", "NGN")).rejects.toThrow(
      "Invalid rate",
    );
  });
  ...
  it("throws PaycrestHttpError on non-ok response", async () => {
    mockFetch({ message: "Unauthorized" }, false, 401);
    await expect(adapter.getRate("USDC", "100", "NGN")).rejects.toBeInstanceOf(
      PaycrestHttpError,
    );
  });
});
