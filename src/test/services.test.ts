...
describe("TransactionService", () => {
  ...
  it("should require updates for update operation", async () => {
    await expect(service.updateTransaction("tx_123", {})).rejects.toThrow(
      "No updates provided",
    );
  });

  it("should require transaction ID for deletion", async () => {
    await expect(service.deleteTransaction("")).rejects.toThrow(
      "Transaction ID is required",
    );
  });
});
...
describe("ServiceContainer", () => {
  ...
  it("should provide all default services", () => {
    ...
    expect(container.getTransactionService()).toBeInstanceOf(
      TransactionService,
    );
  });
