...
describe("ErrorHandler", () => {
  ...
  describe("handle method", () => {
    ...
    it("should include message field when available", async () => {
      const body = await parseBody(
        ErrorHandler.handle(new Error("Test message")),
      );
  ...
  describe("serverError method", () => {
    ...
    it("should use generic message in production", async () => {
      vi.stubEnv("NODE_ENV", "production");
      const body = await parseBody(
        ErrorHandler.serverError(new Error("Sensitive error")),
      );
    ...
    it("should include actual message in development", async () => {
      vi.stubEnv("NODE_ENV", "development");
      const body = await parseBody(
        ErrorHandler.serverError(new Error("Detailed error")),
      );
  ...
  describe("security filtering", () => {
    it("should sanitize file paths", async () => {
      vi.stubEnv("NODE_ENV", "development");
      const body = await parseBody(
        ErrorHandler.handle(new Error("Error in /path/to/file.ts")),
      );
    ...
    it("should sanitize connection strings", async () => {
      ...
      const body = await parseBody(
        ErrorHandler.handle(
          new Error("Connection failed: mongodb://user:pass@host/db"),
        ),
      );
    ...
    it("should sanitize API keys", async () => {
      ...
      const body = await parseBody(
        ErrorHandler.handle(
          new Error("API key abc123def456ghi789jkl012mno345pqr failed"),
        ),
      );
