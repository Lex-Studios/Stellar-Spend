...
describe("api key management routes", () => {
  ...
  it("lists API keys", async () => {
    listApiKeysMock.mockResolvedValue([
      { id: "key_1", name: "Partner", status: "active" },
    ]);
  ...
  it("rotates and revokes API keys", async () => {
    rotateApiKeyMock.mockResolvedValue({
      id: "key_2",
      plaintextKey: "ssp_live_new.secret",
    });
    revokeApiKeyMock.mockResolvedValue({ id: "key_1", status: "revoked" });
    listApiKeyUsageMock.mockResolvedValue([
      { id: "usage_1", apiKeyId: "key_1" },
    ]);
    ...
    const revokeResponse = await revokePOST(
      makeRequest("/api/api-keys/key_1/revoke", "POST", {
        reason: "compromised",
      }),
      { params: Promise.resolve({ id: "key_1" }) },
    );
