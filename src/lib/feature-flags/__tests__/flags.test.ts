...
describe("Feature Flag Schema", () => {
  it("parses default flags", () => {
    ...
    expect(flags.corridors.brazilBrl).toEqual({
      enabled: false,
      percentage: 0,
    });
  });
...
describe("resolveGradualRollback", () => {
  ...
  it("returns false when rollout is not enabled", () => {
    expect(
      resolveGradualRollback({ enabled: false, percentage: 0.5 }, "user_1"),
    ).toBe(false);
  });

  it("returns true for 100% rollout", () => {
    expect(
      resolveGradualRollback({ enabled: true, percentage: 1 }, "user_1"),
    ).toBe(true);
  });

  it("returns false for 0% rollout", () => {
    expect(
      resolveGradualRollback({ enabled: true, percentage: 0 }, "user_1"),
    ).toBe(false);
  });
  ...
  it("returns false when userId is not provided for partial rollout", () => {
    expect(resolveGradualRollback({ enabled: true, percentage: 0.5 })).toBe(
      false,
    );
  });
});
