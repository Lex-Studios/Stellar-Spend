...
export async function getAllbridgeGasFee(
  ...
  const { Messenger, FeePaymentMethod } =
...
export async function getAllbridgeGasFeeOptions(
  ...
  try {
    ...
    const nativeInt =
      feeOptions[FeePaymentMethod.WITH_NATIVE_CURRENCY]?.[AmountFormat.INT] ||
      "0";
    const nativeFloat =
      feeOptions[FeePaymentMethod.WITH_NATIVE_CURRENCY]?.[AmountFormat.FLOAT] ||
      "0";

    // Get stablecoin fee in both formats
    const stablecoinInt =
      feeOptions[FeePaymentMethod.WITH_STABLECOIN]?.[AmountFormat.INT] || "0";
  ...
  try {
    ...
    if (transferStatus?.status) {
      ...
      if (allbridgeStatus === "completed" || allbridgeStatus === "success") {
        ...
        allbridgeStatus === "processing" ||
        allbridgeStatus === "in_progress"
      ) {
        status = "processing";
      } else if (
        allbridgeStatus === "pending" ||
        allbridgeStatus === "waiting"
      ) {
        status = "pending";
      }
