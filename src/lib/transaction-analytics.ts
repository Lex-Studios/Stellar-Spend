...
export function buildTransactionAnalyticsReport(
  ...
  const finalizedDurations = rows
    .filter(
      (row) => row.finalized_at !== null && row.finalized_at >= row.timestamp,
    )
  ...
  const dailyReports = sortedDays.map((date) => {
    ...
    const successfulTransactions = dayRows.filter(
      (row) => row.status === "completed",
    ).length;
    const failedTransactions = dayRows.filter(
    ...
    const durations = dayRows
      ...
      .map((row) => row.finalized_at! - row.timestamp);
    const totalFees = dayRows.reduce(
      (sum, row) => sum + toNumber(row.total_fee),
      0,
    );
  ...
  const feeTrends = sortedDays.map((date) => {
    const dayRows = dailyBuckets.get(date) ?? [];
    const totalFeesCollected = dayRows.reduce(
      (sum, row) => sum + toNumber(row.total_fee),
      0,
    );
    ...
    return {
      ...
      averagePaycrestFee: average(
        dayRows.map((row) => toNumber(row.paycrest_fee)),
      ),
  ...
  return {
    ...
    timing: {
      ...
      averageTransactionTimeSeconds: average(
        finalizedDurations.map((value) => value / 1000),
      ),
    },
    fees: {
...
export async function getTransactionAnalytics(
  periodDays = 7,
): Promise<TransactionAnalyticsReport> {
  const safePeriodDays =
    Number.isFinite(periodDays) && periodDays > 0 ? Math.floor(periodDays) : 7;
