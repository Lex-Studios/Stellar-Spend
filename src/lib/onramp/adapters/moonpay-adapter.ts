...
export class MoonpayAdapter implements DepositProviderAdapter {
  ...
  async getQuote(
    ...
    if (!response.ok)
      throw new Error(`MoonPay quote error: ${response.statusText}`);
    const data = await response.json();

    const destinationAmount = String(
      parseFloat(data.quoteCurrencyAmount || "0"),
    );
    const fee = String(
      parseFloat(request.fiatAmount) -
        parseFloat(data.baseCurrencyAmount || request.fiatAmount) || "0",
    );
    ...
    return {
      ...
      rate:
        parseFloat(data.quoteCurrencyAmount || "0") /
          parseFloat(request.fiatAmount) || 0,
      fee,
      estimatedTime: 60,
  ...
  async createOrder(
    ...
    if (!response.ok)
      throw new Error(`MoonPay order error: ${response.statusText}`);
    const data = await response.json();
  ...
  async getOrderStatus(
    ...
    if (!response.ok)
      throw new Error(`MoonPay status error: ${response.statusText}`);
    const data = await response.json();
