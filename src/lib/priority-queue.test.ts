...
vi.mock("@/lib/webhook/retry-scheduler", () => ({
  ...
  hasRemainingAttempts: vi.fn(
    (record: any) => record.attemptCount < record.maxAttempts,
  ),
}));
...
describe("DeliveryRetryQueue", () => {
  ...
  it("tracks by-destination metrics", () => {
    ...
    expect(
      retryQueue.getMetrics().byDestination["https://hook1.example.com"],
    ).toBe(2);
  });
...
describe("calculatePriorityFee", () => {
  it("applies fee multiplier for URGENT", () => {
    expect(calculatePriorityFee("100", TransactionPriority.URGENT)).toBe(
      "150.000000",
    );
  });

  it("applies fee multiplier for LOW", () => {
    expect(calculatePriorityFee("100", TransactionPriority.LOW)).toBe(
      "80.000000",
    );
  });
