...
export class EmailAdapter implements ChannelAdapter {
  ...
  async send(
    ...
    if (!endpoint) {
      return {
        status: "skipped",
        errorMessage: "EMAIL_NOTIFICATION_ENDPOINT not configured",
      };
    }
    ...
    const response = await fetch(endpoint, {
      ...
      body: JSON.stringify({
        from:
          process.env.EMAIL_NOTIFICATION_FROM ?? "noreply@stellar-spend.local",
        to,
        subject,
        text: message,
      }),
...
export class SmsAdapter implements ChannelAdapter {
  ...
  async send(
    ...
    const enabled =
      process.env.SMS_NOTIFICATION_ENABLED?.toLowerCase() === "true";
    if (!enabled || !endpoint) {
      return {
        status: "skipped",
        errorMessage: "SMS notifications not configured",
      };
    }
...
export class PushAdapter implements ChannelAdapter {
  ...
  async send(
    ...
    if (!endpoint) {
      return {
        status: "skipped",
        errorMessage: "PUSH_NOTIFICATION_ENDPOINT not configured",
      };
    }
