...
export async function notifyTransactionStatusUpdate(
  ...
  const prefs = await getOrCreateNotificationPreferences(
    context.transaction.userAddress,
  );
  ...
  await Promise.all(
    channels.map(async (channel) => {
      ...
      let result = await adapter
        .send(destination, template.subject, template.message)
        .catch((err: unknown): DeliveryResult => ({
          status: "failed",
          errorMessage: err instanceof Error ? err.message : String(err),
        }));
...
export async function getTransactionNotificationDeliveries(
