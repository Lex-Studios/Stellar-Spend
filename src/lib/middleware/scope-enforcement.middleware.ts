...
export function enforceScope(request: NextRequest, apiKey: ApiKeyRecord): NextResponse | null {
  ...
  if (!hasRequiredScope(apiKey, requiredScope)) {
    ...
    auditLoggingService.logAction('authorization.denied', 'api_key', 'failure', {
