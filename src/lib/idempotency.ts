...
function getConfig() {
  ...
  const lockTtlMs = Number.parseInt(
    process.env.IDEMPOTENCY_LOCK_TTL_MS ?? "",
    10,
  );
  ...
  return {
    ...
    lockTtlMs:
      Number.isFinite(lockTtlMs) && lockTtlMs > 0
        ? lockTtlMs
        : DEFAULT_LOCK_TTL_MS,
  };
}
...
async function beginIdempotency(
  ...
  try {
    ...
    await client.query("DELETE FROM idempotency_keys WHERE expires_at <= $1", [
      now,
    ]);

    const existing = await fetchRecordForUpdate(
      client,
      idempotencyKey,
      method,
      path,
...
export async function withIdempotency(
  ...
  try {
    ...
    try {
      ...
      console.error(
        "Failed to clear idempotency record after persistence error:",
        cleanupError,
      );
    }
  }
