...
describe("getRequiredScope", () => {
  it("returns read:quotes for GET /api/v1/offramp/quote", () => {
    expect(getRequiredScope("GET", "/api/v1/offramp/quote")).toBe(
      "read:quotes",
    );
  });

  it("returns write:payouts for POST /api/v1/offramp/execute-payout", () => {
    expect(getRequiredScope("POST", "/api/v1/offramp/execute-payout")).toBe(
      "write:payouts",
    );
  });
  ...
  it("returns read:transactions for GET /api/transactions", () => {
    expect(getRequiredScope("GET", "/api/transactions")).toBe(
      "read:transactions",
    );
  });

  it("returns write:transactions for POST /api/transactions", () => {
    expect(getRequiredScope("POST", "/api/transactions")).toBe(
      "write:transactions",
    );
  });
});
