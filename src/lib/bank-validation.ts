...
export type CountryValidatorFn = (
  field: BankFieldType,
  value: string,
 ...
 */
export function registerCountryValidator(
  countryCode: string,
  fn: CountryValidatorFn,
): void {
 ...
 */
export function getCountryValidator(
  countryCode: string,
): CountryValidatorFn | undefined {
...
export function validateAccountNumber(value: string): ValidationResult {
  ...
  if (digits.length < 4)
    return {
      valid: false,
      error: "Account number is too short (min 4 digits).",
    };
  if (digits.length > 20)
    return {
      valid: false,
      error: "Account number is too long (max 20 digits).",
    };
  return { valid: true };
}
...
export function validateRoutingNumber(value: string): ValidationResult {
  ...
  if (sum % 10 !== 0)
    return { valid: false, error: "Invalid routing number (checksum failed)." };
  return { valid: true };
}
...
export function validateIBAN(value: string): ValidationResult {
  ...
  if (!/^[A-Z]{2}\d{2}[A-Z0-9]{1,30}$/.test(iban)) {
    return {
      valid: false,
      error:
        "IBAN format is invalid. Expected: CC##BBAN (e.g. GB29NWBK60161331926819).",
    };
  }
  // MOD-97 check
 ...
 */
export function validateBankFieldForCountry(
  country: string,
  field: BankFieldType,
  value: string,
): ValidationResult {
    ...
    if (fieldConfig) {
      ...
      if (field === "account") {
        ...
        if (cfg.pattern && !new RegExp(cfg.pattern).test(digits)) {
          return {
            valid: false,
            error: `Account number must match format for ${country}`,
          };
        }
        if (cfg.minLength && digits.length < cfg.minLength) {
          return {
            valid: false,
            error: `Account number too short (min ${cfg.minLength})`,
          };
        }
        if (cfg.maxLength && digits.length > cfg.maxLength) {
          return {
            valid: false,
            error: `Account number too long (max ${cfg.maxLength})`,
          };
        }
        return { valid: true };
      }
      if (field === "routing") {
        ...
        if (cfg.pattern && !new RegExp(cfg.pattern).test(digits)) {
          return {
            valid: false,
            error: `Routing number must match format for ${country}`,
          };
        }
        if (cfg.length && digits.length !== cfg.length) {
          return {
            valid: false,
            error: `Routing number must be ${cfg.length} digits`,
          };
        }
        return { valid: true };
      }
...
function validateNigeriaBankField(
  _field: BankFieldType,
  value: string,
): ValidationResult | null {
...
function validateIndiaBankField(
  field: BankFieldType,
  value: string,
): ValidationResult | null {
  ...
  if (!/^[A-Z]{4}0[A-Z0-9]{6}$/.test(ifsc)) {
    return {
      valid: false,
      error:
        "IFSC code must match format: 4 letters + 0 + 6 alphanumeric (e.g. HDFC0001234).",
    };
  }
  return { valid: true };
}
