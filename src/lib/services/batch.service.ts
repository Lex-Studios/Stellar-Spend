...
export async function getBatchProgress(
  ...
  const completed = transactions.filter(
    (t: BatchTransaction) => t.status === "completed",
  ).length;
  const failed = transactions.filter(
    (t: BatchTransaction) => t.status === "failed",
  ).length;
  const pending = transactions.filter(
    (t: BatchTransaction) => t.status === "pending",
  ).length;
  return {
    batchId,
    total,
...
export async function getBatchAnalytics(
  ...
  return {
    ...
    successRate:
      totalTransactions > 0
        ? Math.round((completedTx / totalTransactions) * 100)
        : 0,
    avgBatchSize:
      totalBatches > 0 ? Math.round(totalTransactions / totalBatches) : 0,
  };
}
