...
export async function createTable(): Promise<void> {
  ...
  try {
    ...
    throw new DeliveryStoreError(
      "Failed to create delivery_records table",
      err,
    );
  }
}
...
export async function updateRecord(
  ...
  updates: Partial<
    Pick<
      DeliveryRecord,
      "status" | "attemptCount" | "attempts" | "nextAttemptAt"
    >
  >,
): Promise<DeliveryRecord> {
  ...
  for (const [key, value] of Object.entries(updates)) {
    if (key in columnMap) {
      const dbValue =
        key === "attempts" ? JSON.stringify(value) : (value ?? null);
...
export async function getRecordsByStatus(
  ...
  try {
    ...
    throw new DeliveryStoreError(
      `Failed to query delivery records with status ${status}`,
      err,
    );
  }
}
