...
export async function POST(request: NextRequest) {
  ...
  if (
    ...
    (typeof body.rateLimitMaxRequests !== "number" ||
      body.rateLimitMaxRequests <= 0)
  ) {
    return ErrorHandler.validation(
      "rateLimitMaxRequests must be a positive number",
    );
  }
  ...
  if (
    ...
    return ErrorHandler.validation(
      "rateLimitWindowMs must be a positive number",
    );
  }
  ...
  try {
    const apiKey = await createApiKey({
      ...
      rateLimitMaxRequests:
        typeof body.rateLimitMaxRequests === "number"
          ? body.rateLimitMaxRequests
          : undefined,
      rateLimitWindowMs:
        typeof body.rateLimitWindowMs === "number"
          ? body.rateLimitWindowMs
          : undefined,
      expiresAt:
