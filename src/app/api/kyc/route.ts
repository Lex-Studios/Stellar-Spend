...
export async function GET(req: NextRequest) {
  try {
    ...
    if (action === "compliance-report") {
      ...
      const to = parseInt(
        req.nextUrl.searchParams.get("to") || String(Date.now()),
      );
  ...
  try {
    ...
    if (action === "upload-document") {
      ...
      return NextResponse.json({
        success: true,
        upload,
        kyc: KYCLimitService.getKYC(userId),
      });
    }
    ...
    return NextResponse.json(
      { error: 'action must be "submit", "upload-document", or "aml-screen"' },
      { status: 400 },
    );
  } catch (error) {
    return NextResponse.json(
...
export async function PATCH(req: NextRequest) {
  try {
    const { action, userId, reason, requestedTier, requestId } =
      await req.json();
    ...
    if (action === "verify") {
      ...
      if (!kyc)
        return NextResponse.json(
          { error: "No KYC submission found for user" },
          { status: 404 },
        );
      return NextResponse.json({ success: true, kyc });
    }
    ...
    if (action === "reject") {
      if (!reason)
        return NextResponse.json(
          { error: "reason is required to reject KYC" },
          { status: 400 },
        );
    ...
    if (action === "request-limit-increase") {
      if (!requestedTier)
        return NextResponse.json(
          { error: "requestedTier is required" },
          { status: 400 },
        );
      const request = KYCLimitService.requestLimitIncrease(
        userId,
        requestedTier,
      );
    ...
    if (action === "approve-limit-increase") {
      ...
      return NextResponse.json({
        success: true,
        limits: KYCLimitService.getUserLimits(userId),
      });
    }
