...
export async function POST(req: NextRequest) {
  try {
    const authorId =
      req.headers.get("x-user-address") ?? req.headers.get("x-admin-id");
    if (!authorId) {
    ...
    if (!disputeId || !content) {
      return NextResponse.json(
        { error: "disputeId and content are required" },
        { status: 400 },
      );
    }
    ...
    const note = await disputeRepository.addNote(
      disputeId,
      authorId,
      content,
      internal,
...
export async function GET(req: NextRequest) {
  try {
    ...
    const disputeNotes = await disputeRepository.getNotes(
      disputeId,
      includeInternal,
    );
