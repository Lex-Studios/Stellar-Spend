    ...
    try {
      ...
      for (const tx of transactions) {
        ...
        for (const field of REQUIRED_FIELDS) {
          if (
            tx[field as keyof typeof tx] === undefined ||
            tx[field as keyof typeof tx] === null
          ) {
            return ErrorHandler.validation(
              `Missing required field in transaction: ${String(field)}`,
            );
          }
        }
        ...
        if (transaction.userAddress.toLowerCase() !== wallet.toLowerCase()) {
          return ErrorHandler.validation(
            "Transaction wallet does not match request wallet",
          );
        }
