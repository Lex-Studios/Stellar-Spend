...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to fetch enforcement policy" },
      { status: 500 },
    );
  }
}
...
export async function PUT(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to update enforcement policy" },
      { status: 500 },
    );
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!userId || typeof enforce !== "boolean") {
      return NextResponse.json(
        { error: "userId and enforce (boolean) are required" },
        { status: 400 },
      );
    }

    const result = enforce
      ? TwoFAService.enforce(userId)
      : TwoFAService.unenforce(userId);
    if (!result) {
      return NextResponse.json(
