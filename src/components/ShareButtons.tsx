...
function generateReceiptImage(
  ...
  if (privacy.includeAmount)
    row(
      "AMOUNT",
      `${amount}${privacy.includeCurrency ? ` ${currency}` : ""}`,
      true,
    );
...
export function ShareButtons({
  ...
  const buildShareText = () => {
    ...
    if (privacy.includeCurrency)
      parts.push(privacy.includeAmount ? currency : `(${currency})`);
    ...
    <div className="space-y-3">
      ...
      {showPrivacy && (
        <div className="p-3 border border-[#333333] bg-[#111111] space-y-2 text-xs">
          <p className="text-[#777777] uppercase tracking-widest text-[10px]">
            Privacy Controls
          </p>
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              onChange={(e) =>
                setPrivacy((p) => ({ ...p, includeAmount: e.target.checked }))
              }
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              onChange={(e) =>
                setPrivacy((p) => ({ ...p, includeCurrency: e.target.checked }))
              }
              className="accent-[#c9a962]"
            />
