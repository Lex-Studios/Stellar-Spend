...
function exportAnalyticsCSV(analytics: AnalyticsPeriod) {
  const rows = [
    ...
    ...analytics.spendingPatterns.map((sp) => [
      sp.date,
      sp.transactionCount,
      sp.amount,
    ]),
...
export function AnalyticsDashboard({ userAddress }: AnalyticsDashboardProps) {
  ...
  const [prevAnalytics, setPrevAnalytics] = useState<AnalyticsPeriod | null>(
    null,
  );
      ...
      {trendValues.length > 0 && (
        <div className="bg-white p-6 rounded-lg border">
          ...
          <p className="text-xs text-gray-500 mb-3">
            Daily volume (USD) over selected period
          </p>
      ...
      <div className="bg-white p-6 rounded-lg border">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold">
              {feeAnalysis.averageFeePercentage.toFixed(2)}%
            </p>
          </div>
          <div>
        ...
        <div className="mt-4 p-3 bg-green-50 rounded-lg border border-green-200">
          <p className="text-sm text-green-800 font-medium">
            💡 Estimated savings from USDC fee optimization:{" "}
            <strong>${feeSavings}</strong>
          </p>
        </div>
      </div>
            ...
            >
              <div>
                ...
                <p className="text-xs text-gray-500">
                  {sp.transactionCount} transactions
                </p>
              </div>
